...
import BranchServicesPage from '../public/branch/BranchServicesPage';
import ServiceDetailPage from '../public/branch/ServiceDetailPage';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Services Page Content</h1>
          <p className="text-gray-600">Edit public services pages (per location)</p>
        </div>
      ...
      >
        <div className="w-full h-full overflow-y-auto" style={{ maxHeight: 'calc(100vh - 300px)' }}>
          <div className="relative">
            {selectedServiceId ? (
              <ServiceDetailPage
                key={`${selectedBranchId || 'services'}_${selectedServiceId}`}
                embedded={true}
                cmsEditMode={editMode}
                cmsBranchId={selectedBranchId || null}
                cmsBranchName={branchName}
                cmsBranchSlug={branchSlug}
                cmsServiceId={selectedServiceId}
                onBack={() => setSelectedServiceId(null)}
              />
            ) : (
              <BranchServicesPage
                key={selectedBranchId || 'services'}
                embedded={true}
                cmsEditMode={editMode}
                cmsBranchId={selectedBranchId || null}
                cmsBranchName={branchName}
        ...
        <div className="fixed inset-0 z-50">
          ...
          <div className="absolute inset-4 bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
              <div className="font-semibold text-gray-900">Services Page</div>
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="relative">
                {selectedServiceId ? (
                  <ServiceDetailPage
                    key={`${selectedBranchId || 'services'}_${selectedServiceId}_fullscreen`}
                    embedded={true}
                    cmsEditMode={editMode}
                    cmsBranchId={selectedBranchId || null}
                    cmsBranchName={branchName}
                  ...
                  <BranchServicesPage
                    key={`${selectedBranchId || 'services'}_fullscreen`}
                    embedded={true}
                    cmsEditMode={editMode}
                    cmsBranchId={selectedBranchId || null}
                    cmsBranchName={branchName}
