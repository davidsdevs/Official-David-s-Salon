    ...
    try {
      ...
      const exportData = filteredStaff.map(member => {
        ...
        weekDates.forEach((date, index) => {
          ...
          if (isStaffOnLeave(member.id || member.uid, date)) {
            row[dayLabel] = 'On Leave';
          } else if (shift && shift.start && shift.end) {
            row[dayLabel] = `${formatTime12Hour(shift.start)} - ${formatTime12Hour(shift.end)}`;
          } else {
            row[dayLabel] = 'Off';
          }
        });
      ...
      link.setAttribute('download', `Staff_Schedule_${formatWeekRange().replace(/ /g, '_')}.csv`);
